...
def label_single_storm(cyclone_id_string, goes_ships_matching_tolerance=GOES_SHIPS_MATCHING_TOLERANCE):
    ...
    assert type(goes_ships_matching_tolerance) is int, f'goes_ships_matching_tolerance must be int; got {type(goes_ships_matching_tolerance)}'
    ...
    for sat_time in ds.satellite_valid_time_unix_sec:
        ...
        if np.abs(time_diffs).min() <= goes_ships_matching_tolerance:
            ...
            RI = int(np.any([
                (ds.ships_storm_intensity_m_s01.sel(ships_valid_time_unix_sec=t) - ref_intensity) >= RI_24_HR_THRESHOLD_KTS * KT_TO_METERS_PER_SECOND for t in valid_times
            ]))
