...
print('Creating training storms\n')
labels = dict()
...
print('Creating validation storms\n')
labels = dict()

for storm_id in range(100, 110):
    print(f'Generating storm {storm_id}')
    storm_samples = []
    storm_labels = dict()
    for satellite_valid_time_unix_sec in range(100):
...
with open(Path.home() / "research_data/GLP/synthetic_data/synth_storms/valid_labels.json", "w") as file:
    json.dump(labels, file, indent=4)
print('Done!')